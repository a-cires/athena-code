...
std::vector<PolarPoint> convert_to_polar(const std::vector<Point>& points, const Point& origin) {
    ...
    for (const auto& point : points) {
        ...
        // Calculate theta (angle) in radians - swap x and y for north-oriented (0 rad = north)
        double theta = std::atan2(centered_x, centered_y);
        
        // Convert from -pi to pi range to 0 to 2*pi range
        if (theta < 0) {
            theta += 2.0 * M_PI;
        }
    ...
    try {
        ...
        for (size_t i = 0; i < polar_points.size(); i++) {
            std::cout << keys[i] << ": r=" << polar_points[i].r 
                     << ", theta=" << polar_points[i].theta << " radians" << std::endl;
        }
