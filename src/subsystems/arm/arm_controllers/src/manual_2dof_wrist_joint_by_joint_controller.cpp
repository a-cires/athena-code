...
{
Manual2DOFWristJointByJointController::Manual2DOFWristJointByJointController() : controller_interface::ControllerInterface() {}

void Manual2DOFWristJointByJointController::logger_function()
{
...
controller_interface::CallbackReturn Manual2DOFWristJointByJointController::on_init()
{
  control_mode_.initRT(control_mode_type::FAST);

  try
  {
    param_listener_ = std::make_shared<manual_2dof_wrist_joint_by_joint_controller::ParamListener>(get_node());
  }
...
controller_interface::CallbackReturn Manual2DOFWristJointByJointController::on_configure(
...
controller_interface::InterfaceConfiguration Manual2DOFWristJointByJointController::state_interface_configuration() const
{
...
controller_interface::CallbackReturn Manual2DOFWristJointByJointController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  for (size_t i = 0; i < command_interfaces_.size(); ++i)
  {
...
PLUGINLIB_EXPORT_CLASS(
  arm_controllers::Manual2DOFWristJointByJointController, controller_interface::ControllerInterface)
