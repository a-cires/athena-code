  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  const double a =  linear_x - angular_z * track_width / 2;
  const double b =  linear_x + angular_z * track_width / 2;
  const double c =  linear_y - angular_z * track_width / 2;
  const double d =  linear_y + angular_z * track_width / 2;

  const double front_left_velocity = (sqrt( pow( b , 2) + pow ( d , 2) ) )*(1/(wheel_radius*M_PI));
  const double front_right_velocity = (sqrt( pow( b , 2) + pow( c , 2 ) ) )*(1/(wheel_radius*M_PI));
  const double rear_left_velocity = (sqrt( pow( a , 2 ) + pow( d , 2) ) )*(1/(wheel_radius*M_PI));
  const double rear_right_velocity = (sqrt( pow( a, 2 ) + pow( c , 2) ) )*(1/(wheel_radius*M_PI));
  ...
  command_interfaces_[4].set_value(front_left_velocity);
